...
ecan_data_file <- "data_catalogue.csv"
...
for (i in (1:nfiles)){
  ...
  odin$date <- as.POSIXct(paste(odin$Day,odin$Time), tz='NZST')
...
# Load ECan's data
ecan_data <- read_csv(paste0(data_folder,ecan_data_file))
ecan_data$date <- as.POSIXct(ecan_data$DateTime,format = "%d/%m/%Y %I:%M:%S %p")
ecan_data$DateTime <- NULL
names(ecan_data) <- c('Station','ws_max','T2m','ws','CO','T','PM10','T6m','PMc','wd','PM2.5','date')
...
for (i in (1:nfiles)){
  ...
  merged_data <- merge(ecan_data.1hr.pm,odin.1hr.pm, by='date',all=TRUE)
